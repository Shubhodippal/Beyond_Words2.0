...
def configure_runtime_device():
    gpus = tf.config.list_physical_devices('GPU')
    if not gpus:
        return False, [], "No TensorFlow-compatible GPU detected."
    try:
        tf.config.set_visible_devices(gpus[0], 'GPU')
        tf.config.experimental.set_memory_growth(gpus[0], True)
        tf.config.set_soft_device_placement(True)
        logical_gpus = tf.config.list_logical_devices('GPU')
        return True, logical_gpus, None
    except RuntimeError as error:
        return False, [], str(error)


GPU_AVAILABLE, GPU_DEVICES, GPU_ERROR = configure_runtime_device()
...
def detect_faces(image):
    ...
    for (x, y, w, h) in faces:
        ...
        roi_gray = (roi_gray / 255.0).astype(np.float32)
...
def main():
    ...
    if not GPU_AVAILABLE:
        st.error(f"GPU-only mode enabled. {GPU_ERROR}")
        st.stop()

    st.caption(f"TensorFlow device: GPU ({GPU_DEVICES[0].name})")
    ...
    if selected_video is not None:
        ...
        if run_detection:
            ...
            while True:
                ...
                if face is not None:
                    with tf.device('/GPU:0'):
                        pred = model(face, training=False).numpy()
